import { Controller, Get, UseGuards, Request, ForbiddenException, Patch, Body, Param, Post, Delete } from '@nestjs/common';
...
export class AdminController {
    ...
    @Patch('users/:id')
    async updateUser(@Request() req: { user: { role: string } }, @Param('id') id: string, @Body() body: { balance?: number; role?: string }) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
    ...
    @Post('announcements')
    async createAnnouncement(@Request() req: { user: { role: string } }, @Body() body: { message: string; type: string }) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
        return this.adminService.createAnnouncement(body.message, body.type);
    }

    @Delete('announcements/:id')
    async deleteAnnouncement(@Request() req: { user: { role: string } }, @Param('id') id: string) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
