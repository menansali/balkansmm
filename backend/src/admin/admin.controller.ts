...
export class AdminController {
    ...
    @Patch('users/:id')
    async updateUser(@Request() req: any, @Param('id') id: string, @Body() body: { balance?: number; role?: string }) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
    ...
    @Post('announcements')
    async createAnnouncement(@Request() req: any, @Body() body: { message: string; type: string }) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
    ...
    async getAllAnnouncements(@Request() req: any) {
        if (req.user.role !== 'admin') throw new ForbiddenException('Admin access only');
