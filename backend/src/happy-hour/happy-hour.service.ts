...
export class HappyHourService {
    ...
    async getActiveHappyHour() {
        ...
        for (const hh of recurringHappyHours) {
            if (hh.recurringStart && hh.recurringEnd) {
                if (currentTime >= hh.recurringStart && currentTime <= hh.recurringEnd) {
    ...
    calculateDiscountedPrice(
        ...
        const categoryMatch = happyHourCategories.length === 0 ||
            happyHourCategories.some(cat => serviceCategory.toLowerCase().includes(cat.toLowerCase()));

        const platformMatch = happyHourPlatforms.length === 0 ||
            happyHourPlatforms.some(plat => serviceCategory.toLowerCase().includes(plat.toLowerCase()));
