...
export class SchedulerController {
    ...
    @Get('posts')
    async getPosts(@Request() req: AuthRequest, @Query('status') status?: string) {
        return this.schedulerService.getScheduledPosts(req.user.userId, status);
    }

    @Get('posts/:id')
    async getPost(@Request() req: AuthRequest, @Param('id', ParseIntPipe) id: number) {
    ...
    async cancelPost(@Request() req: AuthRequest, @Param('id', ParseIntPipe) id: number) {
        return this.schedulerService.cancelPost(req.user.userId, id);
    }

    @Delete('posts/:id')
    async deletePost(@Request() req: AuthRequest, @Param('id', ParseIntPipe) id: number) {
