...
export class AnalyticsService {
  ...
  private extractUsername(platform: string, url: string): string | null {
    const patterns: Record<string, RegExp> = {
      ...
      youtube:
        /(?:youtube\.com\/@|youtube\.com\/c\/|youtube\.com\/channel\/)([a-zA-Z0-9_-]+)/,
  ...
  private async scrapeProfileData(
    platform: string,
    username: string,
  ): Promise<ProfileData> {
    ...
    if (rapidApiKey) {
      try {
        ...
        console.error(
          `Failed to fetch from RapidAPI for ${platform}/${username}:`,
          error,
        );
    ...
    switch (platform) {
      case 'instagram':
        return {
          ...
          avgLikes: this.calculateAvgLikes(
            data.data?.edge_owner_to_timeline_media?.edges || [],
          ),
          avatarUrl:
            data.data?.profile_pic_url_hd || data.data?.profile_pic_url,
        };
      ...
      case 'tiktok':
        return {
          ...
          avgViews:
            data.data?.stats?.videoCount > 0
              ? Math.floor(
                  ((data.data?.stats?.heartCount || 0) /
                    data.data.stats.videoCount) *
                    10,
                )
              : 0,
          avatarUrl:
            data.data?.user?.avatarLarger || data.data?.user?.avatarMedium,
        };
      ...
      case 'twitter':
        return {
          ...
          avatarUrl: data.result?.legacy?.profile_image_url_https?.replace(
            '_normal',
            '_400x400',
          ),
        };
  ...
  private calculateEngagement(followers: number, posts: any[]): number {
    ...
    const totalLikes = posts.reduce(
      (sum, p) => sum + (p.node?.edge_liked_by?.count || 0),
      0,
    );
  ...
  private calculateGrowthPercent(snapshots: Snapshot[]): number {
    ...
    return parseFloat(
      (
        ((latest.followers - previous.followers) / previous.followers) *
        100
      ).toFixed(2),
    );
  }
