...
Respond with ONLY the caption text, nothing else.`;
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a social media marketing expert who creates viral, engaging captions.',
          },
  ...
  private async publishPost(post: { id: number; platform: string }) {
  ...
  private async triggerAutoBoost(post: any) {
    try {
      ...
      console.error(
        `[Scheduler] Auto-boost failed for post ${post.id}:`,
        error,
      );
    }
  }
