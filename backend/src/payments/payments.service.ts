...
export class PaymentsService {
    ...
    if (gateway === 'binance') {
      ...
      try {
        ...
        if (response.data.status !== 'SUCCESS') {
          throw new Error(
            response.data.errorMessage || 'Binance Pay order creation failed',
          );
        }
        ...
        this.logger.error(
          `Binance Pay Order Creation Failed: ${this.sanitizeError(e)}`,
        );
  ...
  async processBinanceWebhook(
    ...
    this.logger.log(
      `Binance webhook received: ${merchantTradeNo} - Status: ${bizStatus}`,
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (user.referredById) {
        const commission = existingTx.amount * 0.1;
        await tx.user.update({
          where: { id: user.referredById },
          data: { affiliateBalance: { increment: commission } },
        });
        this.logger.log(
          `Affiliate commission $${commission} credited to referrer ${user.referredById}`,
        );
      }

      this.logger.log(
        `Deposit completed: $${existingTx.amount} for User ${existingTx.userId}`,
      );
      return { returnCode: 'SUCCESS', returnMessage: 'OK' };
    });
  }
  ...
  async processWebhook(
    ...
    this.logger.log(
      `Processing deposit webhook: $${amount} for User ${userId}`,
    );

    return this.prisma.$transaction(async (tx) => {
      if (txId) {
        const existingTx: any = await tx.transaction.findFirst({
          where: { gatewayTxId: txId },
        } as any);
